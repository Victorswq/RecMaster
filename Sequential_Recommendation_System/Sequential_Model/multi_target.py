...
class Fossil(abstract_model):
    ...
    def __init__(self,model_name="Fossil",
        ...
        # build the dataset
        self.dataset=Data_for_Fossil(data_name=self.data_name,seq_len=self.seq_len,one=self.one)
    ...
    def calculate_loss(self,data):
        ...
        for i in range(self.one):
            pos_scores = torch.mul(output, pos_item_embedding[:,i,:]).sum(dim=1)
...
class Data_for_Fossil(abstract_dataset):
    ...
    def get_data_for_model(self):
        ...
        for user_id,item_list in user_item.items():
            ...
            for i in range(1,length-1-self.one):
                train=item_list[:i+self.one]
                if len(train)>self.seq_len+self.one:
                    train=train[-self.seq_len-self.one:]
                changdu=len(train)-self.one
                train+=[changdu]
        ...
        train_data=np.array(self.pad_sequence(train_data,seq_len=self.seq_len+2+self.one))
...
class trainer():
    ...
    def train_epoch(self,data):
        ...
        for train_data in get_batch(train_data_value,batch_size=self.model.batch_size):
            ...
            seq_item=train_data[:,:-2-self.one]
            pos_item=train_data[:,-2-self.one:-2]
